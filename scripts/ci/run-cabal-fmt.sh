...
if ! git ls-files --exclude-standard --no-deleted --deduplicate '*.cabal' | CABAL_FMT="$cabal_fmt" xargs -L 1 sh -c 'echo "$0" && "$CABAL_FMT" -c "$0" 2>/dev/null || ("$CABAL_FMT" -i "$0" && exit 1)'; then
    exit 1
fi
