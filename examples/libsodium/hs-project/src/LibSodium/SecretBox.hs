...
-- 'Binding.Combinators.toHighLevelPure' rather than
-- 'Binding.Combinators.toHighLevel' and no 'IO' reaches the type. What stays in 'IO'
...
import Binding.Combinators (input2, output, toHighLevel, toHighLevelPure)
import Binding.Combinators.Auto (autoChecked, autoMaybe, autoResult)
import Binding.Combinators.Defaults (DefaultIn (..))
import Binding.Combinators.Marshaller (at)
