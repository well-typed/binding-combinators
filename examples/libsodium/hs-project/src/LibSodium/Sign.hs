...
--
-- All five close their spec with 'Binding.Combinators.toHighLevelPure', the multipart
-- pair included: the shared @crypto_sign_state@ enters the spec as a
-- 'Binding.Combinators.scratch' bracket, so those bindings are one spec too. See
-- 'signMultipart'.
--
...
import Binding.Combinators (fixed, input, input2, output, resultIO, scratch,
                            throwOnNonZero, toHighLevel, toHighLevelPure)
import Binding.Combinators.Auto (autoChecked, autoInputs, autoWith)
import Binding.Combinators.Defaults (DefaultIn (..))
import Binding.Combinators.Marshaller (at, unmarshalOutPure)
import Binding.Combinators.Marshaller.Utils (byteStringOut, unsafeByteStringIn,
