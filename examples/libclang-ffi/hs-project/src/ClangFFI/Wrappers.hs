...
-- @Binding.Combinators@ combinators plus the by-value plumbing the raw imports need
...
import Binding.Combinators (fixed, input, output, scratch, toHighLevel)
import Binding.Combinators.Auto (auto, autoInputs, autoResult, checkedResult)
import Binding.Combinators.Defaults (defaultIn, defaultOut)
import Binding.Combinators.Marshaller (Unmarshaller (..), unmarshalOutPure)
import Binding.Combinators.Unlifted (bracketUnlifted, outputUnlifted)
...
-- | The same signature at the raw C types, which do have a @ToFunPtr@ instance.
type RawVisitor = Ptr Void -> Ptr Void -> IO CInt

foreign import capi safe "hs_visit.h hs_visitChildren"
  c_hs_visitChildren :: R CXCursor_ -> FunPtr RawVisitor -> IO CUInt

-- | @clang_visitChildren@. 'withFunPtrAs' passes the 'Visitor' as a
-- 'RawVisitor', which avoids a @foreign import "wrapper"@ of our own. Each stack
-- cursor is copied to the heap, and an exception from the callback is stashed and
-- re-thrown after the call, since it cannot cross the C frame.
visitChildren ::
...
visitChildren parent visit = do
    ...
    withFunPtrAs @RawVisitor wrapped $ \fp ->
