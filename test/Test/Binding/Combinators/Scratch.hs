...
-- | Hidden C arguments the high-level type does not expose: 'scratch' and
-- 'scratchArray' (buffers C writes into), 'fixed' (a pinned constant),
-- and 'nullConst' (a pinned NULL pointer).
module Test.Binding.Combinators.Scratch (tests) where
...
import Binding.Combinators (fixed, input, output, resultPure, scratch,
                            scratchArray, toHighLevel)
import Binding.Combinators.Marshaller (Unmarshaller, scalar)
import Binding.Combinators.Marshaller.Utils (nullConst, peekIncompleteArrayOut)

import Test.Binding.Combinators.Util (cAdd)
...
hsScratch :: IO Int
hsScratch = toHighLevel writeScratch
          $ scratch (allocaBytes 16)
          $ resultPure fromIntegral

hsWriteBuffers :: IO (IA.IncompleteArray CInt, Int)
hsWriteBuffers = toHighLevel writeBuffers
               $ scratchArray @CChar 16
               $ output intArrOut
               $ resultPure (\arr c -> (arr, fromIntegral c))
  where
    intArrOut :: Unmarshaller (Ptr CInt) (IA.IncompleteArray CInt)
    intArrOut = peekIncompleteArrayOut 3

-- | 'fixed' pins the first C argument to @100@; the high-level type exposes only the
-- second.
hsFixed :: Int -> IO Int
hsFixed = toHighLevel cAdd
        $ fixed (100 :: CInt)
        $ input (scalar (fromIntegral :: Int -> CInt))
        $ resultPure fromIntegral

hsIsNull :: IO Int
hsIsNull = toHighLevel reportNull (fixed nullConst $ resultPure fromIntegral)
