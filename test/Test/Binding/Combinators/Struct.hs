...
-- 'unmarshalOptional' on the read side, and the spec adapters 'asArgument' \/
...
module Test.Binding.Combinators.Struct (tests) where
...
import Binding.Combinators (asArgument, asArgumentC, asOutput, asResult, input,
...
import Binding.Combinators.Marshaller (MarshalStruct, UnmarshalStruct, at,
                                       ...
                                       withStruct, (>>>))
import Binding.Combinators.Marshaller.Utils (nullConst, useAsByteStringLenIn,
                                             withCStringIn)
...
hsTakesOuterConst = toHighLevel c_takesOuterConst
                  $ input (asArgumentC outerIn)
                  $ resultPure fromIntegral
