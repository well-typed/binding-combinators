...
module Test.Binding.Combinators.Unlifted (tests) where
...
import Binding.Combinators (input, resultPure, toHighLevel)
...
-- has a high-level argument after the unlifted output.
...
tests = testGroup "unlifted by-value marshallers"
    [ testCase "outputUnlifted: an unlifted W out-parameter reads back" $
        hsByValueOut 21 >>= (@?= 21)
    , testCase "outputUnlifted: a high-level argument may follow an unlifted output" $
...
hsByValueOut = toHighLevel callFillOut
             $ input          (scalar (CInt . fromIntegral))
             $ outputUnlifted byOutUnmarshaller
             $ resultPure (\v _ -> v)

-- | An unlifted output with a high-level argument /after/ it. This works only because
...
hsByValueOutThenArg = toHighLevel callFillOutFirst
                    $ outputUnlifted byOutUnmarshaller
                    $ input          (scalar (CInt . fromIntegral))
                    $ resultPure (\v _ -> v)
...
hsByValueIn n = do
    boxed <- mkBoxed n
    toHighLevel callReadRef
      ( input (bracketUnlifted onBoxed)
