...
module Test.Binding.Combinators.Errors (tests) where
...
import Binding.Combinators (input, output, resultIO, resultPure, throwOn,
                            throwOnNonZero, throwOnOut, toHighLevel)
import Binding.Combinators.Marshaller (bracket, scalar)

import Test.Binding.Combinators.Util (assertThrows, peekIntOut, returnsStatus)
...
tests = testGroup "errors"
    ...
    , testCase "an assembler over an output: status 0 keeps just the output" $
        hsOutDrop 0 >>= (@?= 7)

    , testCase "an assembler over an output: a non-zero status throws" $
    ...
    , testCase "a throw unwinds the brackets opened before it" $ do
        ...
        let wrapped = toHighLevel returnsStatus
...
hsCheckStatus = toHighLevel returnsStatus
              $ input (scalar id)
              $ throwOnNonZero SampleErr

hsThrowOnNegBool :: CInt -> IO Bool
hsThrowOnNegBool = toHighLevel returnsStatus
                 $ input (scalar id)
...
hsCheckOut = toHighLevel writeOutCode
           $ input (scalar id)
...
-- | An output alongside an error check: the assembler sees the output and the
-- status together and does both jobs, throwing on a non-zero status and otherwise
-- keeping the value.
hsOutDrop :: CInt -> IO Int
hsOutDrop status = toHighLevel (outWithStatus status)
                 $ output peekIntOut
                 $ resultIO (\v s -> if s == 0 then pure v else throwIO (SampleErr s))

hsResultIOMaybe :: CInt -> IO Int
hsResultIOMaybe = toHighLevel returnsStatus
                $ input (scalar id)
                $ resultIO convert
  where
    convert :: CInt -> IO Int
