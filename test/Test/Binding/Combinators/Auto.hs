...
module Test.Binding.Combinators.Auto (tests) where
...
import Binding.Combinators (ToHighLevel, input, output, resultPure,
                            scratchArray, toHighLevel)
import Binding.Combinators.Auto (AutoOutputs, auto, autoChecked, autoInputs,
                                 autoMaybe, autoResult, autoWith, checkedResult,
                                 eitherResult, maybeResult)
import Binding.Combinators.Defaults (DefaultIn (..), defaultOut, defaultRes)
import Binding.Combinators.Marshaller (Unmarshaller, scalar, unmarshalOutPure,
                                       unmarshalOutWith)

import Test.Binding.Combinators.Util (Callback, cAdd, cBsLen, c_strlen,
...
tests = testGroup "auto and defaults"
    ...
    , testGroup "integral widths, in all three combinators"
        ...
        , testCase "the nullable rule over the identity: Maybe (Ptr a)" $ do
            hsMaybePtr True  >>= (\r -> assertBool "a non-NULL return is Just" (isJust r))
...
-- | The @qrcodegen_encodeText@ wrapper, combinator for combinator: one explicit
-- input, one scratch buffer, one output, and 'auto' for the whole remaining run of
-- mundane inputs and then the result.
...
-- | The out-parameters use a marshaller polymorphic in its Haskell type. The
...
-- them, which means the C function must take its out-parameters first. That is the
-- usual C convention, and 'outThenIn' follows it.
...
-- outputs and ignore the C return\": 'Binding.Combinators.discardResult'
...
-- same way, which is what 'Binding.Combinators.Defaults' promises and what a binding
