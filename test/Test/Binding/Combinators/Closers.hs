...
-- as a pure function.
module Test.Binding.Combinators.Closers (tests) where
...
import Binding.Combinators (discardResult, input, output, resultIO, resultPure,
                            toHighLevel, toHighLevelPure)
import Binding.Combinators.Auto (auto)
import Binding.Combinators.Marshaller (scalar)

import Test.Binding.Combinators.Util (cAdd, peekIntOut, returnsStatus)
