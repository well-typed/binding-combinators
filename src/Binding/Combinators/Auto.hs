...
-- 'Binding.Combinators.output' or 'Binding.Combinators.fixed' that C takes /after/ the
...
--
-- The table stops at three merged combinators rather than one per closer because
-- 'autoInputs' composes with any closer at all, so @'autoInputs' '$' 'eitherResult'
-- classify@ and @'autoInputs' '$' theCloserYouWrote@ are both perfectly good specs.
-- The three named ones come up often enough to deserve a word.
--
-- Reach for 'auto' first.
--
-- == How it decides
--
...
--     with that type's 'Binding.Combinators.Defaults.defaultIn' and carry on;
...
--
-- 'auto' never writes an 'Binding.Combinators.output' or a
-- 'Binding.Combinators.scratch', because it has nothing to go on. A C
...
-- typed holes\" in "Binding.Combinators".
--
-- == When something does not line up
--
-- The four type errors 'auto' reports are collected under \"Reading the type errors\"
-- in "Binding.Combinators", each with its cause and its fix. The messages themselves
-- are in "Binding.Combinators.Internal.Errors".
--
module Binding.Combinators.Auto (
...
import Binding.Combinators (AssembleOutputs, ToHighLevel, UncurryOutputs,
                            inputN, resultIO, resultPure)
import Binding.Combinators.Defaults (DefaultIn (..), DefaultRes (..))
import Binding.Combinators.Internal.Errors (AutoMismatch, AutoOutputsMismatch,
...
-- 'Binding.Combinators.Defaults.DefaultRes', so an @int@ closes to 'Int' if the
...
-- 'Binding.Combinators.resultPure' and 'Binding.Combinators.resultIO' take a function
...
-- promise something 'Binding.Combinators.resultIO' cannot: its assembler receives
-- outputs that have already been read, whereas the continuation here receives
-- @pending@ and may decline to run it.
onReturn
  :: (c -> IO (Outputs os) -> IO r)
     -- ^ the C return value, and the deferred read-backs
  -> ToHighLevel os (IO c) (IO r)
onReturn decide =
  ToHighLevel $ \pending cFn -> cFn >>= \c -> decide c pending
{-# INLINE onReturn #-}
...
-- combinator rather than something you can write with 'Binding.Combinators.resultIO',
...
-- something to free, close it with 'Binding.Combinators.resultIO' and do the check
...
-- @get_last_error@ call. To ignore the return value entirely, pass
-- @\\_ -> pure ()@.
--
checkedResult
  :: forall os c hs.
...
-- can write with 'Binding.Combinators.resultPure':
--
...
-- nothing worth copying out. Closing with @'Binding.Combinators.resultPure' (\\m s ->
...
class AutoInputs lo hi lo' hi' | lo hi -> lo' hi' where
  ...
  --
  -- The tail is an ordinary spec, so it does not need to be only a closer. Anything the
  -- high-level type does not take an argument for can go there, an
  -- 'Binding.Combinators.output' in particular.
  --
  ...
  -- @'Binding.Combinators.input' 'defaultIn'@ /is/ the step 'autoInputs' takes, once,
...
{-------------------------------------------------------------------------------
  ...
  *above* them. In practice that means an 'Binding.Combinators.output' is fine (C puts
  ...
  use 'autoInputs' with an explicit closer there.
-------------------------------------------------------------------------------}

-- | Fill the high-level arguments from the signature, then close with a result
-- function of your own: @'autoInputs' '.' 'Binding.Combinators.resultPure'@.
--
...
-- 'Binding.Combinators.resultPure' does:
--
...
-- @'autoInputs' '$' 'Binding.Combinators.resultIO' f@, which reads the same and keeps
...
-- 'Binding.Combinators.resultPure' cannot do.
--
...
-- functions. That is what these classes are for: a combinator of your own,
...
-- taking the varying part as an argument, and needs none of this;
-- @docs\/writing-a-combinator.md@ works that case through on libgit2's handle
-- constructors, and reaches these four only at the end. These four are for a combinator
...
-- > resultPure / resultIO over a caller's assembler   UncurryOutputs os
--
...
-- /determined/ by it, because GHC cannot infer a type family's inputs from its result.
-- Name such a type elsewhere. The 'AutoInputs' constraint normally names it already.
--
...
-- 'Binding.Combinators.UncurryOutputs' to add. Four errors later you have rebuilt
...
-- single line: with outputs it would need an 'AutoOutputs' or 'UncurryOutputs' beside
-- it, as @gitCall@ does.
--
-- == Parameterising one
--
