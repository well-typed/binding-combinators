...
-- 'toHighLevelPure' runs a spec the way 'Binding.Combinators.toHighLevel' does but hands
...
module Binding.Combinators.Result (
...
import Binding.Combinators.Internal.Spec (ToHighLevel, toHighLevel)
...
-- what lets 'Binding.Combinators.Auto.auto' work under 'toHighLevelPure'.
--
...
-- @IO (a -> b)@. Close that one with 'Binding.Combinators.toHighLevel' and strip the
-- 'IO' yourself.
...
-- pure function. 'Binding.Combinators.toHighLevel' with the 'IO' taken off the
...
-- value is produced with 'unsafePerformIO'.
--
-- Pair it with a non-throwing closer, too. A throwing closer under 'toHighLevelPure'
-- yields a pure value that throws when forced, so the exception surfaces wherever the
-- result is first used rather than at the call, which is rarely what a caller expects
-- and is hard to place once it happens. Where the C call can fail, close it with
-- 'Binding.Combinators.toHighLevel' and keep the 'IO'.
--
...
-- the type says, and it is what lets 'Binding.Combinators.Auto.auto' read the
