...
-- 'Binding.Combinators.input' drops it into the former, 'at' aims it at the latter.
...
-- "Binding.Combinators.Marshaller.Utils"; to lift a whole low-level /function/, see
-- "Binding.Combinators".
--
module Binding.Combinators.Marshaller (
    -- * The marshaller type
    Marshal (..)
  , at
  , scalar
...
-- @f@ then @g@. The function side uses 'Binding.Combinators.input' (or
...
-- in "Binding.Combinators.Unlifted".
--
...
-- applies the C arguments itself, so its arity has to match the wrapped
-- marshaller's, and the same combinator handles one- or many-argument defaults:
--
...
-- 'Binding.Combinators.asArgument'.
--
...
-- 'Binding.Combinators.output' opens the bracket on the way in and runs the reader
...
-- 'Binding.Combinators.output' \/ 'Binding.Combinators.Unlifted.outputUnlifted'.
type Unmarshaller ::
